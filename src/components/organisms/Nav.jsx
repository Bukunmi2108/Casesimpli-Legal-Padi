...
const Nav = () => {
  ...
  return (
  <header className='bg-white w-full border-b border-gray-400 fixed z-50'>
    <nav className='flex justify-between items-center max-w-8xl mx-auto px-4 py-4 md:py-6'>
      <div className='flex items-center justify-center gap-2'>
        <div onClick={handleMenu}>
          {menuState? <MdMenu className='w-8 h-8 md:w-12 md:h-12'/> : <MdClose className='w-8 h-8 md:w-12 md:h-12'/>}
        </div>
        <Link to={'/'}><span className='h-2xl'>LegalPadi</span></Link>
      </div>

      <Button text={UserRole[role]} type='grey-bg' size='medium'/>
    </nav>

    <nav className={`${(menuState === true)? '-left-96' : 'left-0'} h-fit md:max-h-[calc(100vh-99px)] max-h-[calc(100vh-83px)] overflow-y-auto flex flex-col gap-8 items-start justify-start absolute top-[83px] md:top-[99px] bg-white border border-r-gray-500 px-4 py-4 transition-all ease-in-out duration-400 z-50`}>
      <div className='flex flex-col gap-2 justify-start items-start w-full'>
